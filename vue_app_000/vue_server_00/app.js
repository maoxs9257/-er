...
server.get("/login",(req,res)=>{
  ...
  //2:sql:查询sql语句
  //数据库 库名 表名 列名 小写字母
  var sql = "SELECT id FROM dd_login";
...
server.get("/reg",(req,res)=>{
  ...
  var upwd=req.query.upwd;
  // 创建用户数据 sql语句
  // 先做查询，查询是否已经存在该用户 若存在 停止操作，并且提示
  var sql="SELECT id FROM dd_login WHERE uname=?";
  pool.query(sql,[uname],(err,result)=>{
    if(err)throw err;
    if(result.length==0){
      var sql="INSERT INTO dd_login VALUES (null,?,md5(?))";
      pool.query(sql,[uname,upwd],(err,result)=>{
        if(err)throw err;
